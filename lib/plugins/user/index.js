...
const getUserAndPaging = (() => {
  var _ref9 = _asyncToGenerator(function* (opt = {}) {
    ...
    let users = knex('user').select(['user.id as id', 'user.username as username', 'user.email as email', 'user.telegram as telegram', 'user.password as password', 'user.type as type', 'user.createTime as createTime', 'user.lastLogin as lastLogin', 'user.resetPasswordId as resetPasswordId', 'user.resetPasswordTime as resetPasswordTime', 'account_plugin.port as port']).leftJoin('account_plugin', 'user.id', 'account_plugin.userId').where({ 'user.type': 'normal' }).groupBy('user.id');
...
const changePassword = (() => {
  var _ref11 = _asyncToGenerator(function* (userId, oldPassword, newPassword) {
  ...
  return function changePassword(_x11, _x12, _x13) {
