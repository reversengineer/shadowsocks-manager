...
const addAccount = (() => {
  var _ref = _asyncToGenerator(function* (type, options) {
    ...
    if (type === 1) {
      ...
      const expires = Date.now() + options.limit * timePeriod[type];
      yield knex('account_plugin').insert({
        type,
        userId: options.user,
        port: options.port,
...
const editAccount = (() => {
  var _ref5 = _asyncToGenerator(function* (id, options) {
    ...
    if (options.type === 1) {
      ...
      const expires = Date.now() + options.limit * timePeriod[options.type];
...
const setAccountLimit = (() => {
  var _ref9 = _asyncToGenerator(function* (userId, accountId, orderType) {
      ...
      const getNewPort = function () {
        return knex('webguiSetting').select().where({
          ...
          if (port.random) {
            ...
            const checkIfPortExists = function (port) {
              let myPort = port;
    ...
    accountData.limit = 1;
    accountData.expireTime = expireTime;
    while (accountData.create >= Date.now()) {
      accountData.limit += 1;
      accountData.create -= countTime;
    }
    let port = yield getAccount({ id: accountId }).then(function (success) {
      return success[0].port;
    });
    yield knex('account_plugin').update({
