...
exports.getServers = (req, res) => {
  const userId = req.session.user;
  let servers;
  knex('server').select(['id', 'host', 'name', 'method', 'scale', 'comment']).orderBy('name').then(success => {
    servers = success;
...
exports.getServerPortFlow = (req, res) => {
  ...
  knex('account_plugin').select().where({
    ...
    if (account.type >= 2 && account.type <= 5) {
      ...
      return knex('webguiSetting').select().where({ key: 'account' }).then(success => {
        if (!success.length) {
...
exports.createOrder = (req, res) => {
  ...
  if (orderType === 'week') {
    ...
    type = 3;
  } else if (orderType === 'day') {
    type = 4;
  } else if (orderType === 'hour') {
    type = 5;
  } else if (orderType === 'season') {
    type = 6;
...
exports.executePaypalOrder = (req, res) => {
  paypal.executeOrder(req.body).then(success => {
    res.send(success);
  }).catch(error => {
    res.status(403).end();
  });
